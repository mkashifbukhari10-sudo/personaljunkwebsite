...
### The existing library — unresolved

The 17 photographs in `public/images/` are described in `scripts/seed-images.ts` and `CREDITS.md` as *the site's own crew photography*, and several carry alt text naming *"Junk Services Dubai crew"*.

**Their provenance has not been confirmed to me, and there are observations that warrant checking before more content is built on them:**

- **None of the files carries any EXIF or XMP metadata** — no camera, no capture date, no creator. This proves nothing on its own (the files may simply have been stripped or re-encoded), but no photograph from a real shoot arrives that way by default.
- **`Same-Day Pickup.jpg` contains visibly garbled signage** — nonsense strings where shopfront and wayfinding text should be. It was also already rejected from upload for a different reason: it shows a parcel locker, not a junk collection.
- **`office strip-out.jpg` has illegible text on the workers' ID badges** at a resolution where it would otherwise be readable.

**These are observations, not a conclusion.** See §17 — this is a dependency to resolve, not an accusation. **Until it is resolved, alt text on these images should not assert that the people shown are this business's employees**, because that is the one claim that would be false if the images turn out to be stock or generated.
...
### I-1 · Provenance of the existing 17 photographs — unresolved, and blocking honest alt text

The library is described as the site's own crew photography, and alt text on several images names *"Junk Services Dubai crew"*. **Confirmation is needed of where these images came from.**

Observations that prompt the question are set out in §7: no EXIF or XMP metadata on any file, visibly garbled signage in `Same-Day Pickup.jpg`, and illegible ID-badge text in `office strip-out.jpg`. **None of this is proof** — but the claim being made in the alt text is specific enough that it should rest on something firmer than assumption.

**Three outcomes:**

| If the images are… | Then |
|---|---|
| **Genuine photography of this business** | Nothing changes. Record the provenance so the question is settled. |
| **Licensed stock** | Alt text must stop naming the company's crew (§4). Licence terms must be checked against §6, and a credits-register entry added. |
| **AI-generated** | They must not depict the business as documentary photography (§7). Replacing them with real photography becomes a priority, and the alt text must be corrected in the interim. |

**This is a business-input question, not a judgement.** It matters because the content strategy's entire competitive argument — stated in `keywords.md` — is that this site is more honest and more specific than a field of template-spun competitors. Photography is the most visible place that claim is either kept or broken.
