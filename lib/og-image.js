...
/** Satori's raster decoder is unreliable with WebP/AVIF data URLs. */
async function toSupportedDataUrl(mime, buffer) {
  if (mime === 'image/webp' || mime === 'image/avif') {
    const png = await sharp(buffer).png().toBuffer();
    return toDataUrl('image/png', png);
  }
...
async function loadFile(file) {
  ...
  return toSupportedDataUrl(mimeFromExtension(file), buf);
}
...
async function loadRemote(src) {
  ...
  return toSupportedDataUrl(type.split(';')[0].toLowerCase(), buf);
}
