...
export function cached(fn, keyParts, tags) {
  // Vercel restores Next's data cache between deployments. Payload changes made
  // outside a running Next request cannot call revalidateTag, so an unchanged
  // key could keep an older post list through a fresh deployment. The commit
  // SHA gives every deployment a clean content snapshot while tags still
  // handle edits made through the deployed admin during that deployment.
  const deploymentVersion = process.env.VERCEL_GIT_COMMIT_SHA || process.env.CONTENT_CACHE_VERSION || 'local';
  return unstable_cache(fn, [...keyParts, deploymentVersion], { tags });
}
