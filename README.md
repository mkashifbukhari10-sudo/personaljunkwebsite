# Junk Services Dubai — Next.js + Payload CMS

Junk removal and waste collection site for Junk Services Dubai (junkservicesdubai.com; branded "Junkit" until 2026-09-15). App Router, JavaScript, no CSS framework:
...
npm run seed:images    # uploads public/images/ to Media and fills the photo slots
npm run dev
```
...
- `npm run seed` against the production database once, to create the launch content, then `npm run seed:images`
  with `BLOB_READ_WRITE_TOKEN` set so the photos land in Blob rather than on a local disk.
...
- Brand artwork lives in `public/brand/` and `app/(frontend)/icon.png` + `apple-icon.png`, all derived from
  `public/images/Logo.png` by `node scripts/brand-assets.mjs` (`lib/brand.js` lists the files). Re-run it after
  replacing the logo. Social images are still generated by `app/og/[key]/route.js`, with the logo composited in.
...
- Social images are generated at stable URLs by `app/og/[key]/route.js` (`/og/default`, `/og/service-{slug}`,
  ...
  icons and favicons are the brand monogram on ink, generated by `scripts/brand-assets.mjs` (see "Brand artwork" above).
...
- `npm run seed:images` uploads the photos in `public/images/` (own photography) and `public/images/areas/` (Creative
  ...
  Media by the `seedKey` it stamps on each document and never touches a slot that already has a photo unless run
  with `-- --update`. Files go wherever Payload's storage points: local `media/` without `BLOB_READ_WRITE_TOKEN`,
  Vercel Blob with it. **Photos uploaded locally do not exist on Vercel**: once the Blob store is connected, put the
  token in `.env`, run `npm run seed:images -- --reupload` (same documents, files copied into Blob) and redeploy.
- Local builds on a small machine (WSL, 8 GB): `NEXT_BUILD_CPUS=2` caps the static-generation worker pool
  ...
  250 ms happy-eyeballs connect attempts to Neon failing as `AggregateError: ETIMEDOUT` mid-prerender — the latter
  is now set in `payload.config.ts` itself, so only the CPU cap is still worth passing. Neither is needed on Vercel. After seeding from a script, delete `.next/cache/fetch-cache` or the build reuses stale content.
